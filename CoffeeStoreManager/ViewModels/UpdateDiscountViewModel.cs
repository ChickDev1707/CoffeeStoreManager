...
{
    public class UpdateDiscountViewModel : BaseViewModel
    {
        ...
        public UpdateDiscountViewModel()
        {
            numcheck = 0;
            num = 0;
            count = 0;
            ...
            DiscountList = new ObservableCollection<PhieuUuDai>();
            var list = new ObservableCollection<PhieuUuDai>(DataProvider.Ins.DB.PhieuUuDais);
            ...
            {
                if (num != 0)
                    MyMessageQueue.Enqueue("Error, Chọn ưu đĩa trước khi tra cứu lại!");
                else
                {
